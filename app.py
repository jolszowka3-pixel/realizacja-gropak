...
def generuj_rozpiske_zbiorcza(data_cel, lista_zlecen, lista_odbiorow):
    ...
    if not grupy: html += "<h2 style='text-align:center;'>Brak zadań.</h2>"
    else:
        for (tr, kr), content in grupy.items():
            html += f"<div class='transport-title'>🚚 {tr} / KURS {kr}</div><table><tr><th>KLIENT</th><th>PRODUKTY</th></tr>"
...
with st.sidebar:
    ...
    if is_admin:
        ...
        with st.expander("🛠️ Korekta"):
            kat = st.selectbox("Dział:", ["Produkcja", "Odbiory"])
            if kat == "Produkcja":
                for i, item in enumerate(dane["zrealizowane"]):
                    st.write(f"{item.get('klient')}"); c1, c2 = st.columns(2)
                    if c1.button("↩️ Przywróć", key=f"rp_{i}"): dane["w_realizacji"].append(dane["zrealizowane"].pop(i)); zapisz_dane(dane); st.rerun()
                    if c2.button("❌ Usuń", key=f"dp_{i}"): dane["zrealizowane"].pop(i); zapisz_dane(dane); st.rerun()
        st.divider()
    if can_edit:
    ...
    data_druk = st.text_input("Data do druku:", value=datetime.now().strftime("%d.%m"))
...
if st.session_state.prev_login and not st.session_state.notif_seen:
    try:
        ...
        if nowe:
            st.markdown(f"""<div class="notification-container"><div class="notif-title">🔔 NOWE WPISY OD TWOJEJ OSTATNIEJ WIZYTY:</div>""", unsafe_allow_html=True)
            for n in nowe[:8]: st.markdown(f'<div class="notif-item">{n}</div>', unsafe_allow_html=True)
            if len(nowe) > 8: st.markdown(f"<i>...oraz {len(nowe)-8} innych wpisów.</i>", unsafe_allow_html=True)
            if st.button("✅ Rozumiem, ukryj to"): st.session_state.notif_seen = True; st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    except Exception as e: st.write(f"Błąd powiadomień: {e}")
...
for i in range(7):
    ...
    with cols[i]:
        ...
        for (tr, kr), content in grupy.items():
            all_r = all(it.get('status')=='Gotowe' for it in content["prod"])
            cl = "cal-entry-ready" if (all_r and content["prod"]) else "cal-entry-out"
            lbl = f"{tr}/K{kr}" if tr in ["Auto 1","Auto 2"] else tr
            tooltip = f"ZLECENIA:&#10;"
            for it in content["prod"]: tooltip += f"• {it.get('klient')} ({it.get('szczegoly','')[:20]})&#10;"
            if content["odb"]:
                tooltip += "&#10;🔄 ODBIORY:&#10;"
                for it in content["odb"]: tooltip += f"• {it.get('miejsce')} ({it.get('towar','')[:20]})&#10;"
            st.markdown(f"<div class='{cl}' title='{tooltip}'>{lbl} ({len(content['prod'])+len(content['odb'])})</div>", unsafe_allow_html=True)
...
t_prod, t_odb, t_log, t_dysp = st.tabs(["🏭 Produkcja", "🔄 Odbiory", "🚚 Przyjęcia PZ", "📋 Dyspozycje"])

with t_prod:
    tp1, tp_planned, tp2 = st.tabs(["Aktywne", "📂 Do zaplanowania", "Historia"])
    with tp1:
        hc = st.columns([2,1.2,5,1.2,0.6]); hc[0].write("**Klient**"); hc[1].write("**Termin**"); hc[2].write(f'**{"Produkty" if is_readonly else "Menu"}**'); hc[3].write(f'**{"Status" if is_readonly else "Akcja"}**')
        last_g = None
        for i, z in enumerate(dane["w_realizacji"]):
            ...
            c = st.columns([2,1.2,5,1.2,0.6]); status = z.get('status','W produkcji'); b_st = '<span class="badge-status-ready">✅ GOTOWE</span>' if status=='Gotowe' else '<span class="badge-status-prod">⏳ PROD</span>'
            c[0].markdown(f"**{z.get('klient')}**<br>{b_st}", unsafe_allow_html=True); c[1].write(z.get('termin'))
            u_id = f"z_{i}_{z.get('data_p')}".replace(':','').replace(' ','_')
            if is_readonly: c[2].markdown(f"<div class='readonly-text'>{z.get('szczegoly','-')}</div>", unsafe_allow_html=True)
            else:
                with c[2].popover("Menu"):
                    st.download_button("🖨️ Karta", generuj_html_do_druku(z), f"K_{u_id}.html", "text/html", key=f"dl_{u_id}")
                    nt = st.text_input("Termin", z.get('termin'), key=f"et_{u_id}"); na = st.selectbox("Auto", OPCJE_TRANSPORTU, OPCJE_TRANSPORTU.index(z.get('auto','Brak')), key=f"ea_{u_id}"); nk = st.selectbox("Kurs", [1,2,3,4,5], int(z.get('kurs',1))-1, key=f"ek_{u_id}"); ns = st.text_area("Produkty", z.get('szczegoly',''), key=f"es_{u_id}")
    ...
    with tp_planned:
        for i, z in enumerate(dane["w_realizacji"]):
            ...
            c = st.columns([2,1.2,5,1.2,0.6]); c[0].markdown(f"**{z.get('klient')}**"); u_id = f"pl_{i}_{z.get('data_p')}"
            if is_readonly: c[2].markdown(f"<div class='readonly-text'>{z.get('szczegoly')}</div>", unsafe_allow_html=True)
            else:
...
with t_odb:
    ...
    with to1:
        if not dane["odbiory"]: st.info("Brak zadań.")
        else:
            hc = st.columns([2,1.2,5,1.2,0.6]); hc[0].write("**Dostawca**"); hc[1].write("**Termin**")
            last_o = None
            for i, o in enumerate(dane["odbiory"]):
                ...
                if curr_g != last_o: st.markdown(f"<div class='table-group-header'>📅 {o.get('termin')} | {o.get('auto')}</div>", unsafe_allow_html=True); last_o = curr_g
                c = st.columns([2,1.2,5,1.2,0.6]); c[0].markdown(f"**{o.get('miejsce')}**<br><span class='badge-status-return'>W TOKU</span>", unsafe_allow_html=True); c[1].write(o.get('termin'))
                u_id = f"o_{i}_{o.get('data_p')}".replace(':','').replace(' ','_')
                if is_readonly: c[2].markdown(f"<div class='readonly-text'>{o.get('towar')}</div>", unsafe_allow_html=True)
                else:
                    with c[2].popover("Menu"):
                        nt = st.text_input("Data", o.get('termin'), key=f"ot_{u_id}"); ntow = st.text_area("Co?", o.get('towar',''), key=f"ow_{u_id}")
                        if st.button("💾 Zapisz", key=f"os_{u_id}"): dane["odbiory"][i].update({"termin":nt,"towar":ntow}); zapisz_dane(dane); st.rerun()
...
with t_log:
    ...
    with tl1:
        for i, p in enumerate(dane["przyjecia"]):
            ...
            c = st.columns([2,1.2,5,1.2,0.6]); c[0].write(f"**{p.get('dostawca')}**"); c[1].write(p.get('termin'))
            u_id = f"p_{i}_{p.get('data_p')}"
            if is_readonly: c[2].markdown(f"<div class='readonly-text'>{p.get('towar')}</div>", unsafe_allow_html=True)
            else:
                with c[2].popover("Menu"):
                    nt = st.text_input("Data", p.get('termin'), key=f"pt_{u_id}"); ntw = st.text_area("Towar", p.get('towar'), key=f"pw_{u_id}")
                    if st.button("💾 Zapisz", key=f"ps_{u_id}"): dane["przyjecia"][i].update({"termin":nt,"towar":ntw}); zapisz_dane(dane); st.rerun()
...
with t_dysp:
    ...
    with td1:
        for i, d in enumerate(dane["dyspozycje"]):
            ...
            c = st.columns([2,1.2,5,1.2,0.6]); c[0].write(f"**{d.get('tytul')}**"); c[1].write(d.get('termin'))
            u_id = f"d_{i}_{d.get('data_p')}"

...
if not st.session_state.user:
    ...
    with col_login:
        # TWOJE LOGO NA STRONIE LOGOWANIA
        try:
            st.image("logo.png", use_container_width=True)
        except:
            st.warning("Oczekuję na wczytanie pliku logo.png z GitHuba...")
...
# --- 4. PANEL BOCZNY ---
with st.sidebar:
    # --- TWOJE LOGO W PANELU BOCZNYM ---
    try:
        st.image("logo.png", use_container_width=True)
    except:
        pass # Jeśli plik się ładuje, po prostu na razie go pominie

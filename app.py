...
def generuj_rozpiske_zbiorcza(data_cel, lista_zlecen):
    ...
    for z in z_dnia:
        ...
        grupy[key].append(z)
    if not z_dnia: html += "<h2 style='text-align:center;'>Brak zaplanowanych wysyłek na ten dzień.</h2>"
    else:
        for (tr, kr), items in grupy.items():
            label = f"{tr} / KURS NR {kr}" if tr in ["Auto 1", "Auto 2"] else tr
            html += f"""<div class="transport-block"><div class="transport-title">🚚 {label}</div><table><tr><th class="check">OK</th><th>KLIENT</th><th>SPECYFIKACJA</th><th>ILOŚCI / SZCZEGÓŁY</th><th>STATUS</th></tr>"""
            for it in items:
...
# --- WIDOK DRUKOWANIA (PODGLĄD KARTY) ---
if st.session_state.print_order is not None:
    ...
    with col_btn2: st.info("🖨️ Naciśnij **Ctrl + P** na klawiaturze, aby wydrukować tę kartę.")
...
if not st.session_state.user:
    ...
    with col_login:
        ...
        except: st.warning("Oczekuję na wczytanie pliku logo.png...")
        ...
        with st.form("login_form"):
            u = st.text_input("👤 Login użytkownika"); p = st.text_input("🔒 Hasło dostępu", type="password")
    ...
    if typ == "Produkcja":
        with st.form("f_p", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Zapisz"):
                if kl: dane["w_realizacji"].append({"klient": kl, "termin": tm, "opis": op, "szczegoly": sz, "auto": auto, "kurs": kurs, "pilne": p, "status": "W produkcji", "data_p": datetime.now().strftime("%d.%m %H:%M"), "autor": st.session_state.user}); zapisz_dane(dane); st.rerun()
    elif typ == "Dostawa (PZ)":
